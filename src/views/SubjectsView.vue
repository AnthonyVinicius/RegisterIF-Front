...
<template>
  <BaseLayout>
      ...
      <section class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
        <div v-for="subject in subjects" :key="subject.id"
          ...
          <div class="absolute top-2 right-2 flex gap-1">
            <button @click="openEditModal(subject)" class="text-green-500 hover:text-green-400"
            ...
            <button @click="handleDeleteSubject(subject.id)" class="text-red-600 hover:text-red-800"
          ...
          <p class="text-xs mt-4 text-green-700 font-semibold">
            Curso(s): {{subject.courses?.map(c => c.name).join(", ") || "Sem curso associado"}}
          </p>
        </div>
      </section>
    ...
    <ModalSubject :isOpen="isModalOpen" :isEdit="isEditMode" :itemData="selectedSubject" :courses="courses"
      @close="closeModal" @submit="handleSubmitSubject" />
  </BaseLayout>
</template>